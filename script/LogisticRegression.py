...
@click.option("--penalty",
              default='l2',
              show_default=True,
              help="正则化类型，可选'l1','l2','elasticnet',None")
@click.option("--solver",
              default='lbfgs',
              show_default=True,
              help="优化算法，可选'lbfgs','newton-cg','liblinear','sag','saga'")
...
@click.option("--l1_ratio",
              default=None,
              show_default=True,
              help="ElasticNet混合参数，范围[0,1]，仅在penalty='elasticnet'时使用")
...
@click.option("-o", "--out",
              ...
              required=True,
              help="The output dir")
def logistic(genotype, phenotype, vgenotype, vphenotype, feature, testsize, seed, c, penalty, solver, max_iter,
             l1_ratio, scoring, kfolds, n_bootstrap, out):
    """
    使用逻辑回归进行模型构建
    ...
    if penalty:
        if ',' in penalty:
            param_grid["penalty"] = [p for p in penalty.split(",")]
        else:
            param_grid["penalty"] = [penalty]

    if solver:
        if ',' in solver:
            param_grid["solver"] = [s for s in solver.split(",")]
        else:
            param_grid["solver"] = [solver]

    if max_iter:
        if '_' in max_iter:
            param_grid["max_iter"] = list(np.arange(*[int(i) for i in max_iter.split("_")]))
        elif ',' in max_iter:
            param_grid["max_iter"] = [int(i) for i in max_iter.split(",")]
        else:
            param_grid["max_iter"] = [int(max_iter)]

    if l1_ratio and 'elasticnet' in param_grid.get("penalty", [penalty]):
        if '_' in l1_ratio:
            param_grid["l1_ratio"] = list(np.arange(*[float(i) for i in l1_ratio.split("_")]))
        elif ',' in l1_ratio:
            param_grid["l1_ratio"] = [float(i) for i in l1_ratio.split(",")]
        else:
            param_grid["l1_ratio"] = [float(l1_ratio)]
    ...
    plot_confusion_matrix(best_model, X_test_scale, y_test, f_confusion_matrix_plot, title="LogisticRegression")
